...
<html>
	...
	<body>
		...
		<!-- 
			영역에 저장된 속성에 EL 로 접근할 떄는 속성명이 중복되지 않느 ㄴ경우라면
			EL 내장객체 없이 속성명 만으로 접근할 수 있다.
			즉 requestScope.personObj.name 형태로 기술하지 않아도 된다.
		 -->
		<ul>
			<!-- 
				"속성명.멤버변수" 형태로 기술하면 객체에 선언된 getter()를
				호출하여 값을 출력한다. 즉 아래문장은 getter()호출하므로
				클래스에서 gatName()을 주석처리를 하면 에러가 발생한다.
		...
		//따라서 영역에서 객체를 꺼낸 후 사용시에는 원래의 타입으로
		//형변환 해야한다.
		...
		<ul>
		<!-- EL에서 산술연상이 가능하므로 아래에서는 30이 출력된다.-->
